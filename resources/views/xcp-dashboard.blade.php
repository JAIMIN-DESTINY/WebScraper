...
<head>
    ...
    <title>TxParts Scraping Dashboard For XCellParts</title>
    <style>
        :root {
            --bg: #f6f9fb;
...
<body>
    <main class="page">
        <header class="topbar">
            <h1>TxParts Scraping Dashboard For XCellParts</h1>
            <a class="export-btn" href="{{ route('xcp-products.export') }}">Export All Products</a>
        </header>

        @include('partials.dashboard-switcher', ['activeDashboard' => 'xcp'])

        <section class="metrics" aria-label="Scraping metrics">
            <a class="metric-card {{ $activeTable === 'products' ? 'is-active' : '' }}" href="{{ route('xcp-dashboard', ['table' => 'products']) }}">
